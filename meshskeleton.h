...
class MeshSkeleton {
	...
	LineSegments m_segmentConnectingEdges;
       ...
       public:
	MeshSkeleton(UndirectedGraph &segmentGraph)
	    : m_segmentGraph(segmentGraph) {}
	...
	void connectAdjacentSkeletonParts(
	    Skeleton s, boost::optional<size_t> segmentIndex = boost::none) {
		...
		for (size_t adjacentSegment : adjacentSegments) {
			if (m_skeletonParts[adjacentSegment])  // adjacent
							       // skeleton part
							       // has been
			// computed => connect parts
			{
				...
				auto adjacentSegmentNodePos =
				    adjacentSkeleton.getNodes();
				...
				m_segmentConnectingEdges.add_edge(
				    ...
				    adjacentSegmentNodePos[closestNodes
							       .second]);
			}
		}
	}
	void addSkeletonPart(
	    Skeleton s, boost::optional<size_t> segmentIndex = boost::none) {
	...
	void Draw(Shader *edgeShader, Shader *nodeShader,
		...
		for (auto skeletonPart : m_skeletonParts)
			if (skeletonPart)
				skeletonPart.get().Draw(edgeShader, nodeShader,
							meshModelMatrix);
