...
template <typename TriangleMesh> class ConnectivitySurgeon {
  ...
  SkeletonGraph convert_to_skeleton() {
    ...
    for (size_t i = 0; i < numberOfSkeletonNodes; ++i) {
      ...
      for (size_t vi : record[orig_id]) {
        skeletonGraph[vd].collapsed_vertices.push_back(vi);
        Point3D pvi = m_originalMesh.point(Vertex_index(vi));
        skeletonGraph[vd].collapsed_positions.push_back(pvi);
        p = Point3D(p.x() + pvi.x(), p.y() + pvi.y(), p.z() + pvi.z());
      }
      if (moveNodeToCenterOfMass) {
        double num = double(record[orig_id].size());
        skeletonGraph[vd].point =
            Point3D(p.x() / num, p.y() / num, p.z() / num);
        Point3D originalPosition = m_M.point(Vertex_index(orig_id));
      } else
        skeletonGraph[vd].point = vertex_to_point[orig_id];
    }
    ...
    for (size_t i = 0; i < is_edge_deleted.size(); ++i) {
      if (!is_edge_deleted[i]) {
        ...
        size_t p1_id = static_cast<size_t>(new_vertex_id[p1]);
        size_t p2_id = static_cast<size_t>(new_vertex_id[p2]);
        Skeleton_node p1_vd = id_to_vd[p1_id];
        Skeleton_node p2_vd = id_to_vd[p2_id];

        bool exist;
        Skeleton_edge edge;
        boost::tie(edge, exist) = boost::edge(p1_vd, p2_vd, skeletonGraph);
        if (!exist) {
          boost::add_edge(p1_vd, p2_vd, skeletonGraph);
        }
      }
    }
...
private:
  ...
  void initialize_vertex_to_edge() {
    ...
    for (Vertex_index vi : m_M.vertices()) {
      ...
      for (Halfedge_index hei : CGAL::halfedges_around_source(vi, m_M)) {
  ...
  void initialize_face_to_edge() {
    ...
    for (Face_index fi : m_M.faces()) {
      ...
      for (Edge_index ei : CGAL::edges_around_face(m_M.halfedge(fi), m_M)) {
        assert(size_t(ei) < m_M.number_of_edges());
  ...
  void initialize_edge_to_totalCost() {
    ...
    for (size_t index = 0; index < edge_to_totalCost.size(); index++) {
      ...
      if (totalCost0 < totalCost1) {
        ...
        std::runtime_error("Something is wrong."); // For debugging
                                                   // purposes
  ...
  std::vector<size_t> getNeighbours(size_t ofVertex) {
    ...
    for (size_t ei : edges) {
      ...
      if (i != ofVertex)
        ...
        throw std::runtime_error("Something is wrong. Should have found at "
  ...
  double compute_F(size_t vertexIndex, typename TriangleMesh::Point p) const {
