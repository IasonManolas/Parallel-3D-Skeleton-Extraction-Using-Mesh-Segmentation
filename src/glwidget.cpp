...
void GLWidget::initializeShaders() {
  ...
  segmentShader = new Shader((shaderDirectory + "vertex.glsl").c_str(),
                             (shaderDirectory + "segmfragment.glsl").c_str());
  edgeShader = new Shader((shaderDirectory + "edgevs.glsl").c_str(),
                          (shaderDirectory + "edgefs.glsl").c_str());
...
void GLWidget::mouseMoveEvent(QMouseEvent *event) {
  ...
  if (event->buttons() == Qt::LeftButton) {
    if (mode == defaultView || mode == segmentsView) {
      ...
      // mouseOffset.y());
      scene.handle_mouseMovement(mouseOffset); // TODO create a
                                               // signal function
    }
  }
}
...
void GLWidget::loadModel(std::string filename) {
  activeShader = defaultShader;
  mode = defaultView;
  makeCurrent();
