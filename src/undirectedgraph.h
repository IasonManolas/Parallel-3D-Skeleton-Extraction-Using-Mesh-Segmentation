...
class UndirectedGraph {
	...
	bool add_edge(size_t node1, size_t node2) {
		...
		auto p = boost::add_edge(m_id_to_vd[node1].get(),
					 m_id_to_vd[node2].get(), m_boostGraph);
	...
	std::vector<size_t> getAdjacentNodes(size_t nodeIndex) const {
		...
		std::tie(vit, vend) = boost::adjacent_vertices(
		    m_id_to_vd[nodeIndex].get(), m_boostGraph);
