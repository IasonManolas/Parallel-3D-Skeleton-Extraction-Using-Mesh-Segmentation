...
void Scene::initializeScene() {
  ...
  loadMesh("/home/iason/Models/bunny.obj");
...
void Scene::loadPointSphere() { PS.load("/home/iason/Models/icosahedron.ply"); }
...
void Scene::handle_meshContraction(bool automatic,
  ...
  if (!usingCGALSSkeletonization) {
      ...
      BOOST_FOREACH (halfedge_handle h, halfedges(M.m_M)) {
        if (M.m_M.is_border(h)) {
          ...
          CGAL::Polygon_mesh_processing::triangulate_hole(
              M.m_M, h, std::back_inserter(patch_facets));
        }
      }
    }
    ...
    auto cgalContractionTimeStart = std::chrono::high_resolution_clock::now();
    Skeletonization mcs(M.m_M);
    mcs.contract_until_convergence();
    auto cgalContraactionTimeEnd = std::chrono::high_resolution_clock::now();

    std::cout << "total contraction time using cgal:"
              << std::chrono::duration<double>(cgalContraactionTimeEnd -
                                               cgalContractionTimeStart)
                     .count()
              << std::endl;

    std::cout << "Volume meso skeleton/original:"
              << CGAL::Polygon_mesh_processing::volume(mcs.meso_skeleton()) /
...
void Scene::loadMesh(std::string filename) {
  camera.resetCamera();
  light.changeLightDirection(camera.getPosition());
...
void Scene::handle_saveModel(const std::string destinationDirectory) {
  M.handle_saveModel(destinationDirectory);
}

void Scene::handle_saveSegment(const std::string destinationDirectory) {
