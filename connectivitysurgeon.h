...
class EdgeQueue {
	...
	bool compareEdges(size_t e0, size_t &e1) {
		return m_edge_to_totalCost[e0].first <
		       m_edge_to_totalCost[e1].first;
	}
	void insert(size_t edgeIndex) {
		...
		edgeHeap.erase(std::unique(edgeHeap.begin(), edgeHeap.end()),
			       edgeHeap.end());
		std::sort(edgeHeap.begin(), edgeHeap.end(),
			  [&](size_t ei0, size_t ei1) {
				  return m_edge_to_totalCost[ei0].first <
					 m_edge_to_totalCost[ei1].first;
			  });
	}
	void update() {
		std::sort(edgeHeap.begin(), edgeHeap.end(),
...
class ConnectivitySurgeon {
	...
	// public:
	//  EdgeCompareFunctor(std::vector<std::pair<double, size_t>>
	//  &totalCosts)
	//      : m_edge_to_totalCost(totalCosts) {}
	//};
	...
	std::vector<std::pair<double, size_t>>  // pair.first is the totalCost
						// value
	...
	void initialize_edge_to_vertex() {
		...
		for (Edge_index ei : m_M.edges()) {
			...
			assert(size_t(ei) < m_M.number_of_edges());
			std::vector<size_t> verticesOfEdge{size_t(v0),
							   size_t(v1)};
	...
	void initialize_vertex_to_edge() {
		...
		for (Vertex_index vi : m_M.vertices()) {
			...
			vertex_to_edge[size_t(vi)] =
			    std::move(edgesAroundVertex);
		}
	}
	...
	void initialize_edge_to_totalCost() {
		...
		for (size_t index = 0; index < edge_to_totalCost.size();
		     index++) {
			double totalCost0 = edge_to_shapeCost[index].first +
					    edge_to_shapeCost[index].first;
			double totalCost1 = edge_to_shapeCost[index].second +
					    edge_to_shapeCost[index].second;
			...
			if (totalCost0 < totalCost1) {
				...
				std::runtime_error(
				    "Something is wrong.");  // For debugging
							     // purposes
			edge_to_totalCost[index] =
			    std::make_pair(minCostOfEdge, sourceIndex);
		}
	}
	...
	Matrix4d compute_Q(size_t vertexIndex) {
		...
		for (size_t neighbourIndex : neighbours) {
			Matrix34d Kij =
			    compute_KMatrix(vertexIndex, neighbourIndex);
		...
		for (size_t ei : edges) {
			...
			if (i != ofVertex)
				...
				throw std::runtime_error(
				    ...
				    "least on neighbour.");  // NOTE for debug
							     // purposes
		}
		return neighbours;
	}

	double compute_ShapeCostOfHE(
	    size_t i,
	...
	Matrix34d compute_KMatrix(size_t i, size_t j) const {
		...
		Kernel::Vector_3 edge_vector(Pj.x() - Pi.x(), Pj.y() - Pi.y(),
					     Pj.z() - Pi.z());
		Kernel::Vector_3 a =
		    edge_vector / CGAL::sqrt(edge_vector.squared_length());

		Kernel::Vector_3 b = CGAL::cross_product(
		    a, Kernel::Vector_3(Pi.x(), Pi.y(), Pi.z()));
		Matrix34d K;
		K << 0, -a.z(), a.y(), -b.x(), a.z(), 0, -a.x(), -b.y(), -a.y(),
		    a.x(), 0, -b.z();
		return K;
	}
	...
	void initialize_edge_to_samplingCost() {
		...
		for (size_t edgeIndex = 0;
		     edgeIndex < edge_to_samplingCost.size(); edgeIndex++) {
			size_t i = edge_to_vertex[edgeIndex][0],
			       j = edge_to_vertex[edgeIndex][1];
	...
	double compute_samplingCostOfHE(size_t i, size_t j) {
		auto Pi = vertex_to_point[i], Pj = vertex_to_point[j];
		double ijDistance =
		    CGAL::sqrt((Kernel::Vector_3(Pi, Pj)).squared_length());

		double sumIKDistances = 0;  // sum of distances of the vertex i
					    // to all adjacent vertices of i
		for (size_t neighbourIndex : getNeighbours(i)) {
			Kernel::Vector_3 edge_vector(
			    Pi, vertex_to_point[neighbourIndex]);
			sumIKDistances +=
			    CGAL::sqrt(edge_vector.squared_length());
		}
	...
	void initialize_queue(EdgeQueue &queue) {
		...
		for (size_t edgeIndex = 0; edgeIndex < numberOfEdges;
		     edgeIndex++) {
	...
	void collapse() {
		...
		while (!edgeQueue.empty()) {
			...
			if (is_edge_deleted[ei]) continue;
			if (edge_to_face[ei].size() == 0) continue;

			remove_incident_faces(ei);

			// which of the two vertex should be source so that the
			// halfedges that is
			// deleted is the minimum
			size_t sourceIndex = edge_to_totalCost[ei].second;
			size_t vGoneIndex = edge_to_vertex[ei][sourceIndex];
			size_t vKeptIndex =
			    edge_to_vertex[ei][size_t(!sourceIndex)];

			std::vector<size_t> neighboursOfvGone =
			    getNeighbours(vGoneIndex);
			...
			std::vector<size_t> edgesOfvKept(
			    vertex_to_edge[vKeptIndex]);
			for (size_t i = 0; i < edgesOfvKept.size(); ++i) {
				// ei to be removed
				int ei = edgesOfvKept[i];
				for (size_t j = i + 1; j < edgesOfvKept.size();
				     ++j) {
					int ej = edgesOfvKept[j];
					if (is_same_edge(ei, ej) ||
					    is_edge_deleted[ei]) {
						// look for ei from p2's
						// incident edges
						bool found;
						int ind;
						boost::tie(found, ind) =
						    find_edge(vertex_to_edge
								  [vKeptIndex],
							      ei);
						if (!found) {
							continue;
						...
						remove_edge(vKeptIndex, ei,
							    ind);
						break;
					}
				}
			}
			update_totalCost(vGoneIndex, vKeptIndex,
					 neighboursOfvGone);
	...
	void update_totalCost(size_t vGone, size_t vKept,
		...
		update_Q(
		    vGone,
		    vKept);  // returns a vector containing indices of the edges
			     // of whose the total cost changed

		// the following computes the total cost of an edge if its
		// sampling cost or
		// shape cost changed.This is obviously not optimal but I prefer
		// it since it
		// simplifies the code.
		...
		for (size_t ei : vertex_to_edge[vKept]) {
			processedEdgesSet.insert(
			    ei);  // populate for next for loop
	...
	void update_edge_to_totalCost(size_t edgeIndex) {
		...
		if (totalCost0 < totalCost1)
			edge_to_totalCost[edgeIndex] =
			    std::make_pair(totalCost0, 0);
		else
			edge_to_totalCost[edgeIndex] =
			    std::make_pair(totalCost1, 1);
	}

	void remove_incident_faces(
	    size_t
		edgeIndex) {  // removes incident faces + updates edge_to_face

		// 1)Update edge_to_face for all edges that belong to a face tha
		// will be
		// removed
		for (size_t fid :
		     edge_to_face[edgeIndex]) {  // iterate over all faces that
						 // need to be removed
			is_face_deleted[fid] = true;  // mark them as deleted
			for (size_t j = 0; j < face_to_edge[fid].size();
			     ++j) {  // get the edges of the face that will be
				     // removed
				size_t e = face_to_edge[fid][j];
				if (e == edgeIndex)  // when you fall onto the
						     // edge that will be
						     // deleted
					// continue since for this we don't need
					// to fix
					// edge_to_face
					continue;
				for (size_t k = 0; k < edge_to_face[e].size();
				     ++k) {  // if you find an edge that will
					     // not be removed iterate over
					// its adjacent faces and find the one
					// which will be removed
					if (edge_to_face[e][k] == fid) {
						edge_to_face[e].erase(
						    edge_to_face[e].begin() +
						    k);  // remove it
						break;
					}
				}
	...
	void delete_edge(
		...
		for (size_t i = 0; i < vertex_to_edge[vGone].size(); i++) {
			if (vertex_to_edge[vGone][i] == edgeIndex) {
				vertex_to_edge[vGone].erase(
				    vertex_to_edge[vGone].begin() + i);
				break;
			}
		}
		...
		for (size_t i = 0; i < vertex_to_edge[vKept].size(); ++i) {
			if (vertex_to_edge[vKept][i] == edgeIndex) {
				vertex_to_edge[vKept].erase(
				    vertex_to_edge[vKept].begin() + i);
				break;
			}
		}
	...
	void add_edge(
		...
		for (size_t i = 0; i < vertex_to_edge[vGone].size(); ++i) {
			...
			for (size_t j = 0; j < edge_to_vertex[edge].size();
			     ++j) {
		...
		for (size_t i = 0; i < edge_to_face[ei].size(); ++i) {
			...
			if (!is_face_deleted[fid]) {
				if (std::find(edge_to_face[ej].begin(),
					      edge_to_face[ej].end(),
					      fid) == edge_to_face[ej].end()) {
					edge_to_face[ej].push_back(fid);
					for (size_t j = 0;
					     j < face_to_edge[fid].size();
					     ++j) {
						if (face_to_edge[fid][j] ==
						    ei) {
							face_to_edge[fid][j] =
							    ej;
							break;
	...
	void remove_edge(int v, size_t e, int ind) {
		...
		for (size_t i = 0; i < edge_to_vertex[e].size(); ++i) {
			...
			if (vid != v) {
				for (size_t j = 0;
				     j < vertex_to_edge[vid].size(); ++j) {
					if (vertex_to_edge[vid][j] == e) {
						vertex_to_edge[vid].erase(
						    vertex_to_edge[vid]
							.begin() +
						    j);
						break;
