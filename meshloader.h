...
inline void processNodeAssimp(aiNode *node, const aiScene *scene,
...
inline void loadMeshAssimp(std::string path, std::vector<uint> &indices,
  ...
  if (!scene || scene->mFlags == AI_SCENE_FLAGS_INCOMPLETE ||
    ...
    //            importer.GetImporterInfo(GetImporterCount() << std::endl;
    return;
  }
  //    this->directory = path.substr(0, path.find_last_of('/'));

  std::cout << "Number of Meshes of the root node:"
            << scene->mRootNode->mNumMeshes << std::endl;
  std::cout << "Number of Children of the root node:"
            << scene->mRootNode->mNumChildren << std::endl;
  processNodeAssimp(scene->mRootNode, scene, indices, vertices);
}

inline std::tuple<std::vector<uint>, std::vector<MyVertex>>
