...
CGALSurfaceMesh::halfedge_index getNextHalfedgeInLoop(
    CGALSurfaceMesh M, CGALSurfaceMesh::halfedge_index h) {
	for (CGALSurfaceMesh::halfedge_index hi :
	     CGAL::halfedges_around_source(M.target(h), M)) {
		if (M.is_border(hi)) return hi;
	}
}
std::vector<std::vector<size_t>> getMeshLoops(const CGALSurfaceMesh &M) {
	...
	for (CGALSurfaceMesh::halfedge_index hi : M.halfedges()) {
		if (was_processed[size_t(hi)]) continue;
		was_processed[size_t(hi)] = true;

		if (M.is_border(hi)) {
			std::vector<size_t> verticesInLoop{
			    size_t(M.source(hi))};
			CGALSurfaceMesh::halfedge_index hLoop =
			    getNextHalfedgeInLoop(M, hi);
			was_processed[size_t(hLoop)] = true;
			while (hLoop != hi) {
				verticesInLoop.push_back(
				    size_t(M.source(hLoop)));
...
extractPossibleJunctionNodes(BoostGraph skeleton,
	...
	BOOST_FOREACH (typename BoostGraph::vertex_descriptor vd,
		...
		for (size_t collapseVertexIndex :
		     skeleton[vd].collapsed_vertices) {
			vertex_to_collapsedSkeletonNode[collapseVertexIndex] =
			    vd;
		}
	}
	for (std::vector<size_t> loopVertices : meshLoops) {
		std::set<typename BoostGraph::vertex_descriptor>
		    loopPossibleSkeletonNodes;
		for (size_t loopVertex : loopVertices) {
			loopPossibleSkeletonNodes.insert(
			    vertex_to_collapsedSkeletonNode[loopVertex]);
		}
		possibleBoarderSkeletonNodeGroups.push_back(
		    loopPossibleSkeletonNodes);
	}
...
void Mesh::parallelSkeletonization() {
	...
	std::cout << "Parallel skeletonization took:"
		  << parallelSkeletonizationTime << " sec" << std::endl;
	alphaValue = 0.2;
	m_showContractedSegment = false;
}
void Mesh::runSkeletonizationMethods() {
	if (!CGAL::is_closed(m_M)) {
		std::cout
		    << "MESH IS NOT WATERTIGHT!In order to use skeletonization "
		       "via segmenation the model needs to be "
		       "watertight.Filling "
		       "mesh holes.."
		    << std::endl;
	...
	double segmentationTime =
		...
		.count();
	std::cout << "Segmentation time:" << segmentationTime << " sec"
		  << std::endl;
	...
	std::cout << "skeletonization without using threads took:"
		  << serialSkeletonizationUsingSegmentsTime << " sec"
		  << std::endl;
...
void Mesh::populateVerticesAndIndices(std::__cxx11::string filename) {
	...
	if (m_indices.size() == 0 || m_vertices.size() == 0)
		throw std::string("ERROR:File " + filename +
				  " could not be loaded!");
}
...
CGALSurfaceMesh Mesh::constructSelectedSegmentSurfaceMesh(
