...
class PolygonalMesh {
	...
	void createCGALSurfaceMesh() {
		...
		for (auto const &vert : m_vertices) {
			points.push_back(Kernel::Point_3(
		...
		// std::cout << "Is closed:" << CGAL::is_closed(m_M) <<
		// std::endl;
	}
	...
	void
	updateDrawingVertices() {  // uses M to update std::vector<MyVertex>
		// m_vertices
		for (auto v : m_M.vertices()) {
			CGALSurfaceMesh::Point p = m_M.point(v);
			m_vertices[size_t(v)].Position =
	...
	void printMeshInformation() const {
		std::cout << "Number of m_vertices:" << m_vertices.size()
			  << std::endl;
		std::cout << "Number of faces:" << m_indices.size() / 3
			  << std::endl;
	}
